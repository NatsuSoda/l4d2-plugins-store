...
public OnPluginStart()
{
	...
	l4d_selfhelp_incap = CreateConVar("l4d_selfhelp_incap", "3", " 倒地自救 ,  0:禁止, 1:止痛药、肾上腺素, 2:医疗包, 3:①②选项都可以", FCVAR_NOTIFY);
	l4d_selfhelp_grab = CreateConVar("l4d_selfhelp_grab", "3", " 被烟鬼控制自救,  0:禁止, 1:止痛药、肾上腺素, 2:医疗包, 3:①②选项都可以", FCVAR_NOTIFY);
	l4d_selfhelp_pounce = CreateConVar("l4d_selfhelp_pounce", "3", " 被猎人控制自救, 0:禁止, 1:止痛药、肾上腺素, 2:医疗包, 3:①②选项都可以", FCVAR_NOTIFY);
	l4d_selfhelp_ride = CreateConVar("l4d_selfhelp_ride", "3", " 被猴子控制自救, 0:禁止, 1:止痛药、肾上腺素, 2:医疗包, 3:①②选项都可以", FCVAR_NOTIFY);
	l4d_selfhelp_pummel = CreateConVar("l4d_selfhelp_pummel", "3", " 被大牛控制自救, 0:禁止, 1:止痛药、肾上腺素, 2:医疗包, 3:①②选项都可以", FCVAR_NOTIFY);
	l4d_selfhelp_edgegrab = CreateConVar("l4d_selfhelp_edgegrab", "3", " 挂边自救 , 0:禁止, 1:止痛药、肾上腺素, 2:医疗包, 3:①②选项都可以", FCVAR_NOTIFY);
	l4d_selfhelp_eachother = CreateConVar("l4d_selfhelp_eachother", "1", " 倒地互救 , 0: 禁止, 1 :启动", FCVAR_NOTIFY);
	l4d_selfhelp_pickup = CreateConVar("l4d_selfhelp_pickup", "1", " 倒地捡东西 , 0: 禁止, 1 :启动", FCVAR_NOTIFY);
	l4d_selfhelp_kill = CreateConVar("l4d_selfhelp_kill", "1", " 被控自救会杀死控制你的特感", FCVAR_NOTIFY);
	l4d_selfhelp_hintdelay = CreateConVar("l4d_selfhelp_hintdelay", "2", " 弹出提示的延时", FCVAR_NOTIFY);
	l4d_selfhelp_delay = CreateConVar("l4d_selfhelp_delay", "1", " 多少秒后可以自救", FCVAR_NOTIFY);
	l4d_selfhelp_durtaion = CreateConVar("l4d_selfhelp_durtaion", "3", " 自救需要多少秒", FCVAR_NOTIFY);
	l4d_selfhelp_versus = CreateConVar("l4d_selfhelp_versus", "1", " 对抗模式自救, 0:禁止, 1:启动", FCVAR_NOTIFY);	
...
SelfHelp(client)
{
	...
	if(slot!=-1)
	{
		...
		if(slot ==4)
		{
			...
			if(adrenaline)	PrintToChatAll("\x04★ \x05%N \x01使用了 \x04肾上腺素 \x01自救.", client);  
			if(pills)	PrintToChatAll("\x04★ \x05%N \x01使用了 \x04止痛药 \x01自救.", client); 	
		...
		else if(slot==3)
		{
			...
			HelpState[client]=STATE_OK;
			PrintToChatAll("\x04★★ \x05%N \x01使用了 \x04医疗包 \x01自救.", client); 
 ...
 HelpOther(client, helper)
{
	...
	count++;
	if (count > g_hMaxReviveCount.IntValue)
		count = g_hMaxReviveCount.IntValue;
	...
	PrintToChatAll("\x04★★★ \x05%N \x01使用倒地\x04互救\x01,帮助 \x05%N \x01站了起来.", helper, client);  
	SetPlayerStatus(client);//判断玩家是否是黑白.
}
ReviveClientWithKid(client)
{
	new propincapcounter = FindSendPropInfo("CTerrorPlayer", "m_currentReviveCount");
	...
	SetEntityHealth(client, RoundToFloor(GetConVarFloat(revivehealth)*100.0));
	SetPlayerStatus(client);//判断玩家是否是黑白.
}
ReviveClientWithPills(client)
{
...
//已黑白的设置黑白界面.
void SetPlayerStatus(int client)
{
	if(GetEntProp(client, Prop_Send, "m_currentReviveCount") >= g_hMaxReviveCount.IntValue)
		SetEntProp(client, Prop_Send, "m_bIsOnThirdStrike", 1);//设置黑白界面.
}
